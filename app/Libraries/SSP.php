...
class SSP
{
        ...
        if (isset($request['search']) && $request['search']['value'] != '') {
            ...
            for ($i = 0, $ien = count($request['columns']); $i < $ien; $i++) {
                ...
                if ($requestColumn['searchable'] == 'true') {
                    ...
                    //$globalSearch[] = "" . $column['db'] . "::TEXT LIKE " . $binding;
                    $globalSearch[] = "UNACCENT(" . $column['db'] . "::TEXT) ILIKE UNACCENT('%" . str_replace("'", "''", str_replace('"', '\"', $str)) . "%')";
                }
            }
        }
        if (isset($request['columns'])) {
            for ($i = 0, $ien = count($request['columns']); $i < $ien; $i++) {
                ...
                if ($requestColumn['searchable'] == 'true' && $str != '') {
                    $binding = self::bind($bindings, '%' . $str . '%', PDO::PARAM_STR);
                    $columnSearch[] = "UNACCENT(" . $column['db'] . "::TEXT) ILIKE UNACCENT(" . $binding . ")";
                }
            }
        }
        $where = '';
    ...
    public static function complex(array $request, $conn, string $table, string $primaryKey, array $columns, string $whereResult = null, string $whereAll = null): array
    {
        ...
        $table = (substr($table, 0, 4) === '' || substr($table, 0, 1) === '(' ? '' : '') . $table;
        $limit = self::limit($request, $columns);
        $order = self::order($request, $columns);
        $where = self::filter($request, $columns, $bindings);
    ...
    public static function sql_connect($sql_details)
    {
        try {
            $db = @new PDO(
                ($sql_details['driver'] === 'Postgre' ? 'pgsql' : 'mysql') . ":host={$sql_details['host']};dbname={$sql_details['db']}" . ($sql_details['driver'] === 'postgre' ? ";options='--client_encoding=UTF8'" : ''),
                $sql_details['user'],
                $sql_details['pass'],
                array(PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION)
            );
            if ($sql_details['driver'] === 'mysqli') {
                $db->exec("set names utf8");
            } else {
                $db->exec("SET search_path=public,academico,administrativo,archivo_digital,correspondencia,financiero,marketing,principal,tramite_administrativo");
            }

        } catch (PDOException $e) {
            self::fatal(
                "An error occurred while connecting to the database. " . $sql_details['driver'] .
     ...
     */
    public static function _flatten($a, string $join = ' AND ')
    {
        if (!$a) {
            return '';
